...
**🔗 [Open the live, interactive page](https://tariqfnz1-a11y.github.io/Edtech-Road-Map/)**

Clicking that link opens the tool directly — no extra clicks, no source code. The page itself has a small "📖 Read the guide in the README" link near the top and in the footer that brings you back here if you need more context.

No build step, no dependencies, no server required. You can also download `index.html` and open it directly in any modern browser.
...
**Option A — use it online:** open the [live page](https://tariqfnz1-a11y.github.io/Edtech-Road-Map/) — it loads the interactive tool immediately since the file is named `index.html`, which GitHub Pages serves automatically at the root URL.
...
It's named `index.html` specifically so GitHub Pages serves it automatically at the repo's root URL — no filename needed in the link. There are no external assets, fonts, or scripts to manage. If you need to embed it elsewhere, copy the whole file — it isn't designed to be split apart.
...
**v3 (current)**
- Renamed the file to `index.html` so the GitHub Pages root URL opens the interactive tool directly, with no filename needed in the link.
- Added an in-page "📖 Read the guide in the README" link near the top of the page and in the footer, so anyone who lands on the tool and wants more context can get back to this document in one click.
